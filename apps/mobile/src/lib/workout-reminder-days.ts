...
// the active program, profile preferences, and a caller-supplied fallback.
 ...
 *
 * 3. `currentDays` — whatever the caller already has configured for
 *    workout reminder days. The notification store passes its current
 *    value here, avoiding a circular import.
 */
export function resolveWorkoutReminderDays(currentDays: DayOfWeek[]): DayOfWeek[] {
