-- Fix: get_leaderboard_scores now returns a row for EVERY user in p_user_ids,
-- even if they have zero matching workout data.  Each metric branch starts from
-- unnest(p_user_ids) and LEFT JOINs to the workout / set_logs tables so that
-- COALESCE(..., 0) fills in the gaps.
...
BEGIN
  ...
  ELSIF p_metric = 'streak' THEN
    ...
    RETURN QUERY
      ...
      SELECT u.id AS user_id,
             COALESCE(bs.score, 0)::NUMERIC AS score
        FROM unnest(p_user_ids) AS u(id)
        LEFT JOIN best_streak bs ON bs.user_id = u.id;

  ELSIF p_metric = 'prs' THEN
    RETURN QUERY
      SELECT u.id AS user_id,
             COALESCE(COUNT(sl.id), 0)::NUMERIC AS score
        FROM unnest(p_user_ids) AS u(id)
        LEFT JOIN workout_sessions ws
          ON ws.user_id = u.id
         AND ws.completed_at IS NOT NULL
