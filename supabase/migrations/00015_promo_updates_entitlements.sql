-- ── Update redeem_promo_code to also update entitlements ─────────────
-- Previously the function only inserted into promo_redemptions, so
-- server-side entitlement checks still saw the user as 'free' after
-- a successful promo redemption. This migration replaces the function
-- to also upsert the entitlements row on success.

CREATE OR REPLACE FUNCTION redeem_promo_code(promo_code TEXT, redeemer_id UUID)
RETURNS TABLE(success BOOLEAN, tier TEXT, error_message TEXT)
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
DECLARE
  v_promo RECORD;
BEGIN
  -- Atomically find and increment the promo code in one statement.
  -- The WHERE clause enforces is_active, max_uses, and expiration.
  UPDATE promo_codes
  SET current_uses = current_uses + 1
  WHERE code = promo_code
    AND is_active = TRUE
    AND (max_uses IS NULL OR current_uses < max_uses)
    AND (expires_at IS NULL OR expires_at > NOW())
  RETURNING promo_codes.id, promo_codes.tier
  INTO v_promo;

  -- No matching row means invalid, expired, inactive, or used up
  IF v_promo IS NULL THEN
    RETURN QUERY SELECT FALSE, NULL::TEXT, 'Invalid or expired promo code'::TEXT;
    RETURN;
  END IF;

  -- Per-user duplicate check: insert into redemptions (unique constraint)
  BEGIN
    INSERT INTO promo_redemptions (user_id, promo_code_id)
    VALUES (redeemer_id, v_promo.id);
  EXCEPTION WHEN unique_violation THEN
    -- Roll back the increment since user already redeemed
    UPDATE promo_codes
    SET current_uses = current_uses - 1
    WHERE id = v_promo.id;

    RETURN QUERY SELECT FALSE, NULL::TEXT, 'You have already redeemed this promo code'::TEXT;
    RETURN;
  END;

  -- Update the user's entitlement tier
  UPDATE entitlements
  SET tier = v_promo.tier::entitlement_tier,
      is_trial = false,
      trial_ends_at = null,
      updated_at = now()
  WHERE user_id = redeemer_id;

  -- If no entitlements row exists yet, insert one
  IF NOT FOUND THEN
    INSERT INTO entitlements (user_id, tier, is_trial)
    VALUES (redeemer_id, v_promo.tier::entitlement_tier, false);
  END IF;

  RETURN QUERY SELECT TRUE, v_promo.tier, NULL::TEXT;
END;
$$;
